...
定义：
	...
	所谓「前置声明」（forward declaration）只是类、函数和模板的纯粹声明，而不伴随其定义。代码中用到的那些 symbols, 一般情况下可以使用其前置声明来代替对应的 ``#includes``.

优点：

	* 多余的 ``#includes`` 会害得编译器花费不少时间展开更多头文件，处理大量输入。
	* 而且一旦改动头文件，就必须重新编译整个源代码文件。
